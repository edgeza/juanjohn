...
export default function AlertsPage() {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      // Sort alerts by most recent created_at/timestamp
      ...
      // Keep only the latest ingestion batch
      const latestMs = sorted.length > 0 ? new Date(sorted[0].created_at || sorted[0].timestamp).getTime() : 0;
      // Allow a small window to account for per-row timestamp jitter within one batch (2 minutes)
      const latestOnly = sorted.filter(a => {
        const t = new Date(a.created_at || a.timestamp).getTime();
      ...
      if (latestOnly.length > 0) {
        const latestTs = new Date(latestOnly[0].created_at || latestOnly[0].timestamp).toISOString();
